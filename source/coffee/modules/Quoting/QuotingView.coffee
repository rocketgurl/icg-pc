define [
  ...
  # QuotingView
  # ====
  # Build container view for Quoting functions and do dispatching for
  # actions views.
  class QuotingView extends BaseView
    ...
    buildHtmlElements : ->
      ...
      @$el.append("""
        <div id="quoting-loader-#{@cid}" class="ipm-loader">
          <h2 id="quoting-spinner-#{@cid}"><span>Loading action&hellip;</span></h2>
        </div>
      """)

      # Drop container shim into place
      @$el.append("""
          <div id=\"quoting-container-#{@cid}\" class=\"ipm-container\"></div>
      """)
    ...
    render :
      ...
      (action_view, callback) ->
        ...
        container = @$el.find("#quoting-container-#{@cid}")
    ...
    insert_loader : (msg) ->
      @$el.find("#quoting-loader-#{@cid}").show()
      try
        # Drop in message is present
        if msg?
          $("#quoting-spinner-#{@cid} span").html(msg)

        @loader = @Helpers.loader("quoting-spinner-#{@cid}", 100, '#ffffff')
        @loader.setDensity(70)
        @loader.setFPS(48)
      catch e
        @$el.find("#quoting-loader-#{@cid}").hide()
      this
    ...
    remove_loader : =>
      try
        if @loader? && @loader != undefined
          ...
          # @loader = null
          @$el.find("#quoting-loader-#{@cid}").hide()
          @$el.find("#quoting-spinner-#{@cid} div").remove()
      catch e
        @$el.find("#quoting-spinner-#{@cid} div").remove()
      this
